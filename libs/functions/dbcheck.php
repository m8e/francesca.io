...
if(fra_var["db_conn"] == 1) {
	...
	if(is_numeric(fra_var["db_conn"]) && in_array(fra_var["db_type"], $fra_db_types)) {
		if(fra_var["db_type"] == "sqlite" && !is_writable(fra_var["db_file"])) {
				die("<b>Error!</b> Database Connection Error: <i>sqlite database file not found or not writable in '".fra_var["db_file"]. "'</i>.");
		}
		if(fra_var["db_type"] == "mysql") { 
			...
			if(mysqli_connect_error()) {
				die("<b>Error!</b> Database Connection Error: <i>".mysqli_connect_error(). "</i>.");
			}
		}
	} else {
		die("<b>Error!</b> Wrong or missing 'db conn' configuration.");
	}
}
